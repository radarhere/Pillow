...
function build_brotli {
    ...
    echo "torchstart"
    (cd $out_dir \
        && cmake -DCMAKE_INSTALL_PREFIX=$BUILD_PREFIX -DCMAKE_INSTALL_LIBDIR=$BUILD_PREFIX/lib -DCMAKE_INSTALL_NAME_DIR=$BUILD_PREFIX/lib -DCMAKE_INSTALL_BINDIR=$BUILD_PREFIX/bin $HOST_CMAKE_FLAGS . \
        && echo "torchmid" \
        && make -j4 install)
    echo "torchend"
    touch brotli-stamp
